...
senate_tweets$day <- as.character(round(senate_tweets$created_at, "day"))
timeday <- as.data.frame(table(senate_tweets$day),responseName = 'N of tweets')
rownames(timeday) <- timeday$Var1
timeday <- as.xts(timeday[,-1,drop=FALSE])

# Graph
nobs <- nrow(timeday)
dygraph(timeday[(nobs-100):nobs,],main = 'Number of daily tweets')
```
...
[caltechlogo]: figures/Caltech_LOGO-Orange_RGB.png "Caltech"